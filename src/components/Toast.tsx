...
export default function ToastContainer() {
  ...
  return (
    <div className="fixed top-20 right-4 z-[100] flex flex-col gap-2">
      {toasts.map((t) => (
        <div key={t.id} className={`flex items-center gap-3 px-4 py-3 rounded-xl border shadow-lg animate-slide-in ${colors[t.type]}`}>
          {icons[t.type]}
          <span className="text-sm font-medium text-gray-700">{t.message}</span>
          <button onClick={() => removeToast(t.id)} className="ml-2 p-1 hover:bg-black/5 rounded-full transition"><X className="w-4 h-4 text-gray-400" /></button>
        </div>
      ))}
    </div>
  );
